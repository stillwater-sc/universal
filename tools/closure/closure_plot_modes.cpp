...
int main() {
    ...
    bool success = generatePlots(outputDir + std::string("_encoding_direct"), sw::universal::MappingMode::ENCODING_DIRECT);

    std::cout << "Testing VALUE_CENTERED mode (mathematical layout)..." << std::endl;
    success &= generatePlots(outputDir + std::string("_value_centered"), sw::universal::MappingMode::VALUE_CENTERED);

...
def emit_tables():
    ...
    w('')
    w('// inline: one copy of the tables across every translation unit that includes this header.')
    w('// A namespace-scope constexpr object is const, so it has internal linkage, and each TU would')
    w('// otherwise carry its own 16 KB set.')
    w('inline constexpr unsigned sincos_table_size = %d;' % TABLE_SIZE)
    w('')
    w('// pi/1024')
    w('inline constexpr double pi1024[4] = {')
    w('\t%s' % limbs(pi1024))
    w('};')
    w('')
    w('// sin(k * pi/1024), k = 1..%d' % TABLE_SIZE)
    w('inline constexpr double sin_pi1024[sincos_table_size][4] = {')
    for row in sin_table:
        w('\t{ %s },' % limbs(row))
    w('};')
    w('')
    w('// cos(k * pi/1024), k = 1..%d' % TABLE_SIZE)
    w('inline constexpr double cos_pi1024[sincos_table_size][4] = {')

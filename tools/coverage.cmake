...
if(_compiler_id MATCHES "Clang")
  ...
  set(_proj_source_dir "${_source_dir}")
  if(_proj_source_dir STREQUAL "")
    set(_probe_dir "${BINARY_DIR}")
    while(NOT _probe_dir STREQUAL "" AND NOT _probe_dir STREQUAL "/" AND _proj_source_dir STREQUAL "")
      if(EXISTS "${_probe_dir}/CMakeLists.txt")
        set(_proj_source_dir "${_probe_dir}")
      else()
        get_filename_component(_probe_dir "${_probe_dir}" DIRECTORY)
      endif()
    endwhile()
  endif()

  set(_proj_min_ver "")
  if(NOT _proj_source_dir STREQUAL "" AND EXISTS "${_proj_source_dir}/CMakeLists.txt")
    file(READ "${_proj_source_dir}/CMakeLists.txt" _top_cmake)
    string(REGEX MATCH "cmake_minimum_required[ \t\r\n]*\\([ \t\r\n]*VERSION[ \t\r\n]*([0-9]+\\.[0-9]+(\\.[0-9]+)?)" _min_match "${_top_cmake}")
    if(_min_match)
      set(_proj_min_ver "${CMAKE_MATCH_1}")
    endif()
  endif()

  if(NOT _proj_min_ver STREQUAL ""
     AND CMAKE_VERSION VERSION_GREATER_EQUAL "${_json_required_ver}"
     AND _proj_min_ver VERSION_GREATER_EQUAL "${_json_required_ver}")
    set(_allow_json_parse TRUE)
  endif()

  if(_allow_json_parse)
    message(STATUS "ctest JSON parsing enabled (CMake ${CMAKE_VERSION}, project min ${_proj_min_ver}).")
  else()
    message(STATUS "ctest JSON parsing disabled (CMake ${CMAKE_VERSION}, project min ${_proj_min_ver}).")
  endif()

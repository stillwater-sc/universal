...
A Reference(Real x, const std::vector<std::pair<double, std::uint64_t>>& table) {
	...
	A          r;
	const auto it = std::upper_bound(table.begin(), table.end(), m,
	                                 [](Real v, const std::pair<double, std::uint64_t>& e) { return v < e.first; });
	// the largest exact value <= m; table[0] is zero
	const auto& below = *(it - 1);
	r.setbits(below.second);
	if (below.first != m)
		r.set(0, true);  // inexact: the interval above it, or (maxpos, inf) past the top
	if (negative)
		r.setsign(true);
	return r;
}
...
void VerifyConversion(double x, const std::vector<std::pair<double, std::uint64_t>>& table, Failures& fail) {
	...
	if (static_cast<double>(f) == x && std::isfinite(f))
		fail.check("areal(float " + std::to_string(x) + ")", A(f), expected);
}
...
int VerifyConversions(bool reportTestCases) {
	...
	for (std::size_t i = 0; i < table.size(); ++i) {
		...
		if (i + 1 < table.size())
			xs.push_back(table[i].first / 2.0 + table[i + 1].first / 2.0);
	}
	// the top binade past maxpos, the next binade, and far beyond
	const double nextBinade = std::ldexp(1.0, std::ilogb(top) + 1);
	for (double k = 0.125; k < 8.0; k += 0.125)
		xs.push_back(top + k * ulp);
	for (double t = 0.0; t <= 1.0; t += 0.0625)
		xs.push_back(nextBinade * (1.0 + t));
	for (double x : {2.0 * nextBinade, 3.0 * nextBinade, 1.0e30, 1.0e300})
		xs.push_back(x);
	for (double x : xs) {
...
int VerifyLongDouble(bool reportTestCases) {
	...
	if (std::numeric_limits<long double>::max_exponent > 2000)
		xs.push_back(std::ldexp(1.0l, 2000));
	if (std::numeric_limits<long double>::min_exponent < -2000)
		xs.push_back(std::ldexp(1.0l, -2000));
...
int VerifyNaNPayloads(bool reportTestCases) {
	...
	auto verify = [&](const std::string& what, const A& a, bool quiet) {
		if (a.isnan(quiet ? NAN_TYPE_QUIET : NAN_TYPE_SIGNALLING))
			return;
		++fails;
		if (reportTestCases || fails < 6)
			std::cerr << "FAIL: " << what << " gave " << to_binary(a) << ' ' << a << ", expected a "
			          << (quiet ? "quiet" : "signalling") << " nan\n";
	};
	for (std::uint64_t sign : {std::uint64_t(0), std::uint64_t(1) << 63}) {
		for (std::uint64_t payload :
		     {0x8'0000'0000'0000ull, 0x8'0000'0000'0001ull, 0x8'0000'0000'0002ull, 0xF'FFFF'FFFF'FFFFull, 0x1ull,
		      0x2ull, 0x7'FFFF'FFFF'FFFFull, 0x4'0000'0000'0000ull}) {
	...
	for (std::uint32_t sign : {0u, 1u << 31}) {
		for (std::uint32_t payload :
		     {0x40'0000u, 0x40'0001u, 0x40'0002u, 0x7F'FFFFu, 0x1u, 0x2u, 0x3F'FFFFu, 0x20'0000u}) {
...
inline int VerifyReportedAndWide(bool reportTestCases) {
	...
	}
	{
		...
		for (std::uint64_t u :
		     {std::uint64_t(1) << 33, (std::uint64_t(1) << 33) + 1u, std::uint64_t(1) << 34, ~std::uint64_t(0)}) {
			W x;
			x = u;
...
#	if REGRESSION_LEVEL_1
	...
	nrOfFailedTestCases += ReportTestResult(VerifyConversions<areal<8, 2, std::uint8_t>>(reportTestCases),
	                                        "areal< 8,2,uint8_t >", "conversion");
	nrOfFailedTestCases += ReportTestResult(VerifyConversions<areal<10, 3, std::uint8_t>>(reportTestCases),
	                                        "areal<10,3,uint8_t >", "conversion");
	nrOfFailedTestCases += ReportTestResult(VerifyConversions<areal<12, 4, std::uint16_t>>(reportTestCases),
	                                        "areal<12,4,uint16_t>", "conversion");
	nrOfFailedTestCases += ReportTestResult(VerifyConversions<areal<16, 5, std::uint16_t>>(reportTestCases),
	                                        "areal<16,5,uint16_t>", "conversion");
	nrOfFailedTestCases += ReportTestResult(VerifyConversions<areal<16, 5, std::uint8_t>>(reportTestCases),
	                                        "areal<16,5,uint8_t >", "conversion");
	// every NaN payload stays a NaN of its kind
	nrOfFailedTestCases += ReportTestResult(VerifyNaNPayloads<areal<16, 5, std::uint16_t>>(reportTestCases),
	                                        "areal<16,5,uint16_t>", "nan payload");
	nrOfFailedTestCases += ReportTestResult(VerifyNaNPayloads<areal<16, 5, std::uint8_t>>(reportTestCases),
	                                        "areal<16,5,uint8_t >", "nan payload");
	nrOfFailedTestCases += ReportTestResult(VerifyNaNPayloads<areal<32, 8, std::uint32_t>>(reportTestCases),
	                                        "areal<32,8,uint32_t>", "nan payload");
	nrOfFailedTestCases += ReportTestResult(VerifyNaNPayloads<areal<64, 11, std::uint64_t>>(reportTestCases),
	                                        "areal<64,11,uint64_t>", "nan payload");
	nrOfFailedTestCases += ReportTestResult(VerifyLongDouble<areal<8, 2, std::uint8_t>>(reportTestCases),
	                                        "areal< 8,2,uint8_t >", "long double");
	nrOfFailedTestCases += ReportTestResult(VerifyLongDouble<areal<12, 4, std::uint16_t>>(reportTestCases),
	                                        "areal<12,4,uint16_t>", "long double");
	nrOfFailedTestCases += ReportTestResult(VerifyLongDouble<areal<16, 5, std::uint16_t>>(reportTestCases),
...
#	if REGRESSION_LEVEL_4
	nrOfFailedTestCases += ReportTestResult(VerifyArithmetic<areal<13, 4, std::uint16_t>>(reportTestCases),
	                                        "areal<13,4,uint16_t>", "+ - *");
#	endif

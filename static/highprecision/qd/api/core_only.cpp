// core_only.cpp: the qd arithmetic core, with no other qd header included
//
...
//
// #1334 split qd into core.hpp / manipulators.hpp / iostream.hpp, and a compute kernel is
// meant to be able to include core.hpp alone. For a while it could include it but not
// LINK against it: ulp() in qd_impl.hpp calls ldexp(qd, int), which the impl declared
// and only the mathlib defined (#1462). -fsyntax-only cannot see that, and every other
// test includes the qd.hpp umbrella, which does bring the definition. This one does not:
// it builds a real executable from core.hpp, so the link step is the check.
#include <universal/utility/directives.hpp>
#include <cmath>
#include <iostream>
#include <string>
// the only qd header in this translation unit
...
inline int VerifyCoreFunctionsLink(bool reportTestCases) {
	...
	check(double(ulp(qd(1.0))) == std::ldexp(1.0, -159), "ulp(1)");
	check(double(ulp(qd(1024.0))) == std::ldexp(1.0, 10 - 159), "ulp(1024)");

	int e = 0;
	qd f = frexp(qd(12.0), &e);
	check(double(f) == 0.75 && e == 4, "frexp(12) = 0.75 * 2^4");
	check(ldexp(qd(0.75), 4) == qd(12.0), "ldexp(0.75, 4) = 12");
	check(copysign(qd(3.0), qd(-1.0)) == qd(-3.0), "copysign(3, -1) = -3");
...
try {
	...
	std::string test_suite  = "qd core.hpp without the umbrella (#1462)";

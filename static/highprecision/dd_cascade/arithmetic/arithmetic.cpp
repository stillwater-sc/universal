// arithmetic.cpp: test suite runner of arithmetic operations on double-double cascade (dd_cascade) floating-point
//
...
void TestArithmeticOp(const sw::universal::dd_cascade& a, sw::universal::RandomsOp op, const sw::universal::dd_cascade& b) {
	using namespace sw::universal;
	bool binaryOp = true;
	dd_cascade c;
	switch (op) {
...
namespace sw {
	namespace universal {
		void TestReciprocalIdentity(dd_cascade const& a) {

			dd_cascade oneOverA = reciprocal(a);

			dd_cascade one(1.0);
			dd_cascade error = one - a * oneOverA;
			ReportValue(a, "a", gLabelWidth, gPrecision);
			ReportValue(oneOverA, "1/a", gLabelWidth, gPrecision);
			ReportValue(error, "error", gLabelWidth, gPrecision);
		}

		void TestDivisionalIdentity(sw::universal::dd_cascade const& a) {

			dd_cascade oneOverA = 1.0 / a;
...
try {
	...
	std::string test_suite         = "double-double cascade arithmetic validation";
	std::string test_tag           = "double-double cascade arithmetic";
	...
	// double-double cascade addition
	std::cout << "two sum\n";
	TraceTwoSum(ulp(pow(0.5, 10.0)));
	TraceTwoSum(-ulp(pow(0.5, 10.0)));

	// double-double cascade subtraction
	std::cout << "\ntwo diff\n";
	TraceTwoDiff(ulp(pow(0.5, 10.0)));
	TraceTwoDiff(-ulp(pow(0.5, 10.0)));

	// double-double cascade multiplication
	std::cout << "\ntwo prod\n";
	...
	std::cout << "\n\n\n";
	TestReciprocalIdentity(dd_cascade(1.0));
	TestReciprocalIdentity(dd_cascade(0.5));
	TestReciprocalIdentity(dd_cascade(10.0));
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<dd_cascade>(reportTestCases, RandomsOp::OPCODE_ADD, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<dd_cascade>(reportTestCases, RandomsOp::OPCODE_SUB, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<dd_cascade>(reportTestCases, RandomsOp::OPCODE_MUL, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<dd_cascade>(reportTestCases, RandomsOp::OPCODE_DIV, nrOfRandoms),
		description,
		test_tag
	);

// roundtrip.cpp: test suite for parse/to_string round-trip validation for qd_cascade
//
...
// Helper function to test round-trip conversion for qd_cascade
bool TestRoundTrip(const std::string& input, const std::string& testName, bool reportTestCases) {
    ...
    // Check all 4 components for qd_cascade
    constexpr size_t num_components = 4;  // qd_cascade has 4 components
    ...
    for (size_t i = 0; i < num_components; ++i) {
        ...
        // This is well within quad-double precision but catches real bugs
...
try {
    ...
    std::string test_suite = "qd_cascade parse/to_string round-trip validation";
    ...
    std::vector<std::pair<std::string, std::string>> testCases = {
        ...
        // (e.g., -8.145e+290, -6.682e+274) that exceed double range during the round-trip string parsing.
    ...
    for (const auto& test : testCases) {
        if (!TestRoundTrip(test.first, test.second, reportTestCases)) {
            ++nrOfFailedTestCases;
        }
    }

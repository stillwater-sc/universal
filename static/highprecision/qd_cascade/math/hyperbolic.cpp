// hyperbolic.cpp: test suite runner for hyperbolic functions for quad-double cascade (qd_cascade) floating-point
//
...
try {
	...
	std::string test_suite  = "quad-double cascade mathlib hyperbolic function validation";
	...
	{
		std::cout << "quad-double cascade reference\n";
		qd_cascade x = qdc_pi_4;
		std::cout << "sinh( " << x << " ) = " << sinh(x) << '\n';
		std::cout << "cosh( " << x << " ) = " << cosh(x) << '\n';
		std::cout << "tanh( " << x << " ) = " << tanh(x) << '\n';

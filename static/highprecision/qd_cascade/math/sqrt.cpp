// sqrt.cpp: test suite runner for sqrt function for quad-double floating-point
//
...
namespace sw {
	namespace universal {
		...
		void GenerateSqrtTestCase(Ty fa) {
			...
			//unsigned width = 30;
			Ty fref;
			sw::universal::qd_cascade a, ref, v;
			a = fa;
			fref = std::sqrt(fa);
			ref = fref;
...
try {
	...
	test_tag = "sqrt";
	nrOfFailedTestCases += ReportTestResult(VerifySqrtFunction(reportTestCases, qd_cascade(2.0)), "sqrt(qd_cascade > 1.0)", test_tag);
	nrOfFailedTestCases += ReportTestResult(VerifySqrtFunction(reportTestCases, qd_cascade(0.5)), "sqrt(qd_cascade < 1.0)", test_tag);

	test_tag = "cbrt";
	nrOfFailedTestCases += ReportTestResult(VerifyCbrtFunction(reportTestCases, qd_cascade(2.0)), "cbrt(qd_cascade > 1.0)", test_tag);
	nrOfFailedTestCases += ReportTestResult(VerifyCbrtFunction(reportTestCases, qd_cascade(0.5)), "cbrt(qd_cascade < 1.0)", test_tag);

...
namespace sw { namespace universal { namespace detail {
	...
	int VerifyLogFunction(bool reportTestCases, double maxError = 1.0e-42) {
		...
		// independent: log(2) == tdc_ln2
		{
			TestType v = log(TestType(2.0));
			TestType error = abs(v - tdc_ln2);
			if (error > maxError) {
				++nrOfFailedTestCases;
				if (reportTestCases) ReportLogError("log(2)", v, tdc_ln2, error);
			}
		}
		// independent: log(2^i) == i * tdc_ln2 for exact powers of 2
		for (int i = -52; i < 53; ++i) {
			if (i == 0) continue;
			TestType a = ldexp(TestType(1.0), i);
			TestType ref = tdc_ln2 * TestType(i);
			TestType v = log(a);
	...
	int VerifyLog1pFunction(bool reportTestCases, double maxError = 1.0e-42) {
		...
		// td_cascade has ~156 bits precision, test down to 2^-160
		for (int i = 1; i < 160; ++i) {

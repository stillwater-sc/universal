// pow.cpp: test suite runner for pow function for triple-double cascade (td_cascade) floating-point
//
...
// overload the uniform distribution for triple-double cascade

namespace std
{

	template <>
	class uniform_real_distribution< sw::universal::td_cascade >
	{
	public:
		uniform_real_distribution(const sw::universal::td_cascade& low, const sw::universal::td_cascade& high)
			: _mDist(low[0], high[0])
		{
		}

		template < typename engine >
		sw::universal::td_cascade operator()(engine& eng) {
			return sw::universal::td_cascade(_mDist(eng), 0.5 * std::numeric_limits< double >::epsilon() * _mDist(eng));
		}
...
namespace sw {
	namespace universal {
		...
		void GenerateTestCase(Ty fa, Ty fb) {
			...
			unsigned width = 30;
			Ty fref;
			sw::universal::td_cascade a, b, ref, v;
			a = fa;
			b = fb;
			fref = std::pow(fa, fb);
			ref = fref;
		...
		int calculateNrOfValidBits(const Real& computed, const Real& expected) {
			...
			td_cascade delta = computed - expected;
			if (delta == 0.0) {
				return tdc_max_precision;
			}
			else {
				if (expected == 0.0) {
		...
		int comparePowWithSqrt(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			std::uniform_real_distribution< td_cascade > distribution(1.0, 1048576.0);
			...
			maxValidBits = 0;
			minValidBits = tdc_max_precision;
			for (int i = 0; i < nrOfRandoms; ++i) {
				td_cascade x = distribution(generator);
				td_cascade expected = sqrt(x);
				td_cascade computed = pow(x, td_cascade(0.5));
			...
			if (minValidBits == tdc_max_precision)
				std::cerr << "EXACT ";
			else
		...
		int comparePowWithCubeRoot(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				td_cascade x = distribution(generator);
				td_cascade expected = cbrt(x);
				td_cascade computed = pow(x, tdc_third);
		...
		int comparePowWithSquare(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				td_cascade x = distribution(generator);
				td_cascade expected = x * x;
				td_cascade computed = pow(x, td_cascade(2.0));
		...
		int comparePowWithCube(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				td_cascade x = distribution(generator);
				td_cascade expected = x * x * x;
				td_cascade computed = pow(x, td_cascade(3.0));
		...
		int comparePowWithQuadratic(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				td_cascade x = distribution(generator);
				td_cascade square = x * x;
				td_cascade expected = square * square;
				td_cascade computed = pow(x, td_cascade(4.0));
...
try {
	...
	std::string test_suite  = "triple-double cascade mathlib power function validation";
	...
	std::cout << "Manual testing until we can fix the precision problem of the pow() algorithm\n";
...
/*
 * Release mode
 * triple-double cascade mathlib power function validation: results only
 ...
 * triple-double cascade mathlib power function validation: FAIL
 * 
 * Debug mode
 * triple-double cascade mathlib power function validation: results only
 * PRECISION_THRESHOLD set to 85 bits, which is approximate 25.7635 digits: out of a total of 32 digits
 ...
 * triple-double cascade mathlib power function validation: PASS
 * 

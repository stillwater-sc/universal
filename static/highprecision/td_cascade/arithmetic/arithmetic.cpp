// arithmetic.cpp: test suite runner of arithmetic operations on triple-double cascade (td_cascade) floating-point
//
...
void TestArithmeticOp(const sw::universal::td_cascade& a, sw::universal::RandomsOp op, const sw::universal::td_cascade& b) {
	using namespace sw::universal;
	bool binaryOp = true;
	td_cascade c;
	switch (op) {
...
namespace sw {
	namespace universal {
		void TestReciprocalIdentity(td_cascade const& a) {

			td_cascade oneOverA = reciprocal(a);

			td_cascade one(1.0);
			td_cascade error = one - a * oneOverA;
			ReportValue(a, "a", gLabelWidth, gPrecision);
			ReportValue(oneOverA, "1/a", gLabelWidth, gPrecision);
			ReportValue(error, "error", gLabelWidth, gPrecision);
		}

		void TestDivisionalIdentity(sw::universal::td_cascade const& a) {

			td_cascade oneOverA = 1.0 / a;
...
try {
	...
	std::string test_suite         = "triple-double cascade arithmetic validation";
	std::string test_tag           = "triple-double cascade arithmetic";
	...
	// triple-double cascade addition
	std::cout << "two sum\n";
	TraceTwoSum(ulp(pow(0.5, 10.0)));
	TraceTwoSum(-ulp(pow(0.5, 10.0)));

	// triple-double cascade subtraction
	std::cout << "\ntwo diff\n";
	TraceTwoDiff(ulp(pow(0.5, 10.0)));
	TraceTwoDiff(-ulp(pow(0.5, 10.0)));

	// triple-double cascade multiplication
	std::cout << "\ntwo prod\n";
	...
	std::cout << "\n\n\n";
	TestReciprocalIdentity(td_cascade(1.0));
	TestReciprocalIdentity(td_cascade(0.5));
	TestReciprocalIdentity(td_cascade(10.0));
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<td_cascade>(reportTestCases, RandomsOp::OPCODE_ADD, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<td_cascade>(reportTestCases, RandomsOp::OPCODE_SUB, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<td_cascade>(reportTestCases, RandomsOp::OPCODE_MUL, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<td_cascade>(reportTestCases, RandomsOp::OPCODE_DIV, nrOfRandoms),
		description,
		test_tag
	);

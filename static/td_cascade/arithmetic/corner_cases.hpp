// td_corner_case_tests.hpp: Corner case test infrastructure for triple-double cascade arithmetic
//
 ...
 * --------------------------------------------
 * Triple-double (td_cascade) numbers have ~159 bits of precision (~48 decimal digits), while double
 ...
 * ====================================================
 *
 * Based on the td_cascade implementation structure:
...
// Component verification: check if td_cascade components match expected values within tolerance
inline TestResult verify_components(
    const td_cascade& value,
    double expected_hi,
    double expected_mid,
    double expected_lo,
...
inline TestResult verify_zero(const td_cascade& value, const std::string& test_name = "zero verification") {
...
inline TestResult verify_normalized(const td_cascade& value, const std::string& test_name = "normalization check") {
    // A normalized td_cascade has components in decreasing magnitude order
...
{
    td_cascade sum = a + b;
    td_cascade recovered = sum - b;

    // Allow small error accumulation (within a few ULPs of td_cascade precision)
...
inline td_cascade create_well_separated(double hi_value = 1.0) {
    return td_cascade(hi_value, hi_value * 1e-17, hi_value * 1e-34);
}

// Generate overlapping components (requires heavy renormalization)
inline td_cascade create_overlapping_components(double hi_value = 1.0) {
    return td_cascade(hi_value, hi_value * 0.5, hi_value * 0.25);
}

// Generate value with near-zero lower components
inline td_cascade create_near_zero_lower(double hi_value = 1.0) {
    return td_cascade(hi_value, hi_value * 1e-100, hi_value * 1e-200);
}

// Generate value at ULP boundary
inline td_cascade create_at_ulp_boundary() {
    double one = 1.0;
    double ulp = DOUBLE_EPS;
    return td_cascade(one, ulp, 0.0);
}

// Generate value with mixed signs (tests denormalized inputs)
inline td_cascade create_mixed_signs_internal() {
    return td_cascade(1.0, -1e-17, 1e-34);
}

// Generate tiny value requiring lower components
inline td_cascade create_requires_lower_components() {
    double eps = DOUBLE_EPS;
    return td_cascade(eps / 2.0, eps / 4.0, eps / 8.0);
}

// Generate large magnitude separation
inline td_cascade create_large_magnitude_separation() {
    return td_cascade(1.0e100, 1.0e83, 1.0e66);
}

// Generate small magnitude separation
inline td_cascade create_small_magnitude_separation() {
    return td_cascade(1.0e-100, 1.0e-117, 1.0e-134);
}
    ...
    const std::string& test_name = "self-consistency (a×b)/b=a")
{
    ...
    td_cascade recovered = product / b;
...
{
    td_cascade scaled = a * power_of_2;
...
inline td_cascade create_near_one(double epsilon_scale = 1.0) {
    ...
    const std::string& test_name = "self-consistency (a/b)×b=a")
{
    ...
    td_cascade recovered = quotient * b;
    ...
    const std::string& test_name = "double reciprocal 1/(1/a)=a")
{
    ...
    td_cascade recip = one / a;
    td_cascade double_recip = one / recip;
...
inline td_cascade create_for_reciprocal_test(double scale = 1.0) {
    return td_cascade(scale, scale * 1e-16, scale * 1e-32);
}

// sqrt.cpp: test suite runner for sqrt function for triple-double floating-point
//
...
namespace sw {
	namespace universal {
		...
		void GenerateSqrtTestCase(Ty fa) {
			...
			//unsigned width = 30;
			Ty fref;
			sw::universal::td_cascade a, ref, v;
			a = fa;
			fref = std::sqrt(fa);
			ref = fref;
...
try {
	...
	test_tag = "sqrt";
	nrOfFailedTestCases += ReportTestResult(VerifySqrtFunction(reportTestCases, td_cascade(2.0)), "sqrt(td_cascade > 1.0)", test_tag);
	nrOfFailedTestCases += ReportTestResult(VerifySqrtFunction(reportTestCases, td_cascade(0.5)), "sqrt(td_cascade < 1.0)", test_tag);

	test_tag = "cbrt";
	nrOfFailedTestCases += ReportTestResult(VerifyCbrtFunction(reportTestCases, td_cascade(2.0)), "cbrt(td_cascade > 1.0)", test_tag);
	nrOfFailedTestCases += ReportTestResult(VerifyCbrtFunction(reportTestCases, td_cascade(0.5)), "cbrt(td_cascade < 1.0)", test_tag);

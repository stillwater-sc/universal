...
void Ranges(Real v) {
	using namespace sw::universal;
	using rb10 = rational<10, base16, std::uint16_t>;
	using rb12 = rational<12, base16, std::uint16_t>;
	using rb14 = rational<14, base16, std::uint16_t>;
	using rb20 = rational<20, base16, std::uint32_t>;
	using rb24 = rational<24, base16, std::uint32_t>;
...
try {
	...
	// rb8 and rb16 are the library's base2 aliases: this file is a base16 test, and
	...
	using Exhaustive = rational<8, base16, std::uint8_t>;
	...
	nrOfFailedTestCases += ReportTestResult(ValidateAssignment< rational<4, base16, std::uint8_t> >(reportTestCases), type_tag(rational<4, base16, std::uint8_t>()), test_tag);

	nrOfFailedTestCases += ReportTestResult(ValidateAssignment< rational<8, base16, std::uint8_t> >(reportTestCases), type_tag(rational<8, base16, std::uint8_t>()), test_tag);
#endif

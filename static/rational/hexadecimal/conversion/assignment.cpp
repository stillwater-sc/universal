...
void Ranges(Real v) {
	using namespace sw::universal;
	// every one of these is a base16 rational, so their digit counts are ndigits, not nbits:
	// rb8 and rb16 are the library's base2 aliases and do not belong in this file (#1526)
	using rh10 = rational<10, base16, std::uint16_t>;
	using rh12 = rational<12, base16, std::uint16_t>;
	using rh14 = rational<14, base16, std::uint16_t>;
	using rh20 = rational<20, base16, std::uint32_t>;
	using rh24 = rational<24, base16, std::uint32_t>;
...
try {
	...
	std::string test_suite  = "hexadecimal rational float assignment validation";
	std::string test_tag    = "hexadecimal rational assignment";
	...
	// base16 component holds 16^ndigits of them, so the digit count is what keeps the sweep feasible:
	// 2 digits is 256 encodings per component, 3 digits is 4096 and about 10 minutes (#1526).
	using Sweep = rational<2, base16, std::uint8_t>;
	...
	nrOfFailedTestCases += ReportTestResult(ValidateAssignment< rational<3, base16, std::uint8_t> >(reportTestCases), type_tag(rational<3, base16, std::uint8_t>()), test_tag);
#endif

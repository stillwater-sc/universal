	...
	namespace universal {
		...
		void epsilon() {
			std::cout << std::setw(5) << nbits << "\t" 
				...
				<< std::setw(15) << std::numeric_limits<rational<nbits, base10>>::epsilon() << '\n';
		}
	}
}
...
try {
	...
	std::string test_suite  = "decimal rational precision characterization";
	std::string test_tag    = "decimal rational precision";

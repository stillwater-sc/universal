...
void Ranges(Real v) {
	using namespace sw::universal;
	using rd10 = rational<10, base10, std::uint16_t>;
	using rd12 = rational<12, base10, std::uint16_t>;
	using rd14 = rational<14, base10, std::uint16_t>;
	using rd20 = rational<20, base10, std::uint32_t>;
	using rd24 = rational<24, base10, std::uint32_t>;
...
try {
	...
	nrOfFailedTestCases += ReportTestResult(ValidateNormalization< rational<2, base10, std::uint8_t> >(reportTestCases), type_tag(rational<2, base10, std::uint8_t>()), "normalization");
#endif

...
namespace sw { namespace universal {
	...
	template<typename Real, std::enable_if_t<std::is_floating_point<Real>::value, bool> = true>
	void Experiment() {
		reportConversionError(reportRoundTrip<rd8, Real>(1, 5));
		reportConversionError(reportRoundTrip<rd16, Real>(1, 5));
		reportConversionError(reportRoundTrip<rd32, Real>(1, 5));
		reportConversionError(reportRoundTrip<rd64, Real>(1, 5));
	}
...
try {
	...
	std::string test_suite  = "decimal rational conversion validation";
	std::string test_tag    = "decimal rational conversion";

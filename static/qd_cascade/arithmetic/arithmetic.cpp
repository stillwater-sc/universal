// arithmetic.cpp: test suite runner of arithmetic operations on quad-double cascade (qd_cascade) floating-point
//
...
void TestArithmeticOp(const sw::universal::qd_cascade& a, sw::universal::RandomsOp op, const sw::universal::qd_cascade& b) {
	using namespace sw::universal;
	bool binaryOp = true;
	qd_cascade c;
	switch (op) {
...
namespace sw {
	namespace universal {
		void TestReciprocalIdentity(qd_cascade const& a) {

			qd_cascade oneOverA = reciprocal(a);

			qd_cascade one(1.0);
			qd_cascade error = one - a * oneOverA;
			ReportValue(a, "a", gLabelWidth, gPrecision);
			ReportValue(oneOverA, "1/a", gLabelWidth, gPrecision);
			ReportValue(error, "error", gLabelWidth, gPrecision);
		}

		void TestDivisionalIdentity(sw::universal::qd_cascade const& a) {

			qd_cascade oneOverA = 1.0 / a;
...
try {
	...
	std::string test_suite         = "quad-double cascade arithmetic validation";
	std::string test_tag           = "quad-double cascade arithmetic";
	...
	// quad-double cascade addition
	std::cout << "two sum\n";
	TraceTwoSum(ulp(pow(0.5, 10.0)));
	TraceTwoSum(-ulp(pow(0.5, 10.0)));

	// quad-double cascade subtraction
	std::cout << "\ntwo diff\n";
	TraceTwoDiff(ulp(pow(0.5, 10.0)));
	TraceTwoDiff(-ulp(pow(0.5, 10.0)));

	// quad-double cascade multiplication
	std::cout << "\ntwo prod\n";
	...
	std::cout << "\n\n\n";
	TestReciprocalIdentity(qd_cascade(1.0));
	TestReciprocalIdentity(qd_cascade(0.5));
	TestReciprocalIdentity(qd_cascade(10.0));

	//std::cout << "\n\nfused multiply add\n";
	//a = 1.0; b = 1.0, c = 0.0;
	//c = fma(a, b, c);
	//ReportValue(c, "fma(1.0, 1.0, 0.0, 0.0)");
	//a = 0.0; b = 1.0, c = 1.0;
	//c = fma(a, b, c);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<qd_cascade>(reportTestCases, RandomsOp::OPCODE_ADD, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<qd_cascade>(reportTestCases, RandomsOp::OPCODE_SUB, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<qd_cascade>(reportTestCases, RandomsOp::OPCODE_MUL, nrOfRandoms),
		description,
		test_tag
	);
	...
	nrOfFailedTestCases += ReportTestResult(
		VerifyBinaryOperatorThroughRandoms<qd_cascade>(reportTestCases, RandomsOp::OPCODE_DIV, nrOfRandoms),
		description,
		test_tag
	);

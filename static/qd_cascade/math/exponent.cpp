// exponent.cpp: test suite runner for exponentiation function for quad-double cascade (qd_cascade) floats
//
...
try {
	...
	std::string test_suite  = "quad-double cascade mathlib exponentiation function validation";
	...
	for (int i = 0; i < 30; ++i) {
		...
		qd_cascade exponent = exp(qd_cascade(i));
		qd_cascade error = exponentRef - exponent;

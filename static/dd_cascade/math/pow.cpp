// pow.cpp: test suite runner for pow function for double-double cascade (dd_cascade) floating-point
//
...
// overload the uniform distribution for double-double cascade

namespace std
{

	template <>
	class uniform_real_distribution< sw::universal::dd_cascade >
	{
	public:
		uniform_real_distribution(const sw::universal::dd_cascade& low, const sw::universal::dd_cascade& high)
			: _mDist(low.high(), high.high())
		{
		}

		template < typename engine >
		sw::universal::dd_cascade operator()(engine& eng) {
			return sw::universal::dd_cascade(_mDist(eng), 0.5 * std::numeric_limits< double >::epsilon() * _mDist(eng));
		}
...
namespace sw {
	namespace universal {
		...
		void GenerateTestCase(Ty fa, Ty fb) {
			...
			unsigned width = 30;
			Ty fref;
			sw::universal::dd_cascade a, b, ref, v;
			a = fa;
			b = fb;
			fref = std::pow(fa, fb);
			ref = fref;
		...
		int calculateNrOfValidBits(const Real& computed, const Real& expected) {
			...
			dd_cascade delta = computed - expected;
			if (delta == 0.0) {
				return ddc_max_precision;
			}
			else {
				if (expected == 0.0) {
		...
		int comparePowWithSqrt(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			std::uniform_real_distribution< dd_cascade > distribution(1.0, 1048576.0);
			...
			maxValidBits = 0;
			minValidBits = ddc_max_precision;
			for (int i = 0; i < nrOfRandoms; ++i) {
				dd_cascade x = distribution(generator);
				dd_cascade expected = sqrt(x);
				dd_cascade computed = pow(x, dd_cascade(0.5));
			...
			if (minValidBits == ddc_max_precision)
				std::cerr << "EXACT ";
			else
		...
		int comparePowWithCubeRoot(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				dd_cascade x = distribution(generator);
				dd_cascade expected = cbrt(x);
				dd_cascade computed = pow(x, ddc_third);
		...
		int comparePowWithSquare(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				dd_cascade x = distribution(generator);
				dd_cascade expected = x * x;
				dd_cascade computed = pow(x, dd_cascade(2.0));
		...
		int comparePowWithCube(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				dd_cascade x = distribution(generator);
				dd_cascade expected = x * x * x;
				dd_cascade computed = pow(x, dd_cascade(3.0));
		...
		int comparePowWithQuadratic(bool reportTestCases, int precisionThreshold = PRECISION_THRESHOLD, int nrOfRandoms = NR_RANDOMS) {
			...
			for (int i = 0; i < nrOfRandoms; ++i) {
				dd_cascade x = distribution(generator);
				dd_cascade square = x * x;
				dd_cascade expected = square * square;
				dd_cascade computed = pow(x, dd_cascade(4.0));
...
try {
	...
	std::string test_suite  = "double-double cascade mathlib power function validation";
...
/*
 * Release mode
 * double-double cascade mathlib power function validation: results only
 ...
 * double-double cascade mathlib power function validation: FAIL
 * 
 * Debug mode
 * double-double cascade mathlib power function validation: results only
 * PRECISION_THRESHOLD set to 85 bits, which is approximate 25.7635 digits: out of a total of 32 digits
 ...
 * double-double cascade mathlib power function validation: PASS
 * 

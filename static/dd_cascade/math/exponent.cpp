// exponent.cpp: test suite runner for exponentiation function for double-double cascade (dd_cascade) floats
//
...
void GenerateTestCase(Ty fa) {
	...
	unsigned width = 30;
	Ty fref;
	sw::universal::dd_cascade a, ref, v;
	a = fa;
	fref = std::exp(fa);
	ref = fref;
...
try {
	...
	std::string test_suite  = "double-double cascade mathlib exponentiation function validation";
	...
	for (int i = 0; i < 30; ++i) {
		...
		dd_cascade exponent = exp(dd_cascade(i));
		dd_cascade error = exponentRef - exponent;

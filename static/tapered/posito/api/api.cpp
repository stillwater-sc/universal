// api.cpp: class interface tests for arbitrary configuration posito types
//
...
// Configure the posito template environment
...
// enable/disable error-free serialization I/O
...
try {
	...
	std::cout << "posito class interface tests\n";

	/////////////////////////////////////////////////////////////////////////////////////
	//// posito construction, initialization, assignment and comparisions

	std::cout << "*** posito construction, initialization, assignment, and comparisons\n";
	{
		// maxpos of a posito<8,0> = 64
		posito<8, 0> a(-64), b(128), c(64), d(-64);
...

	{
		...
		posito<16, 2> a{ 0 }, b{ -0.984375f }, c{ 0.984375 }, d{ -0.984375 };
...
#undef LATER
	...
	// arithmetic
	{
		...
		if (nrOfFailedTestCases - start > 0) {
			cout << "FAIL: min/max\n";
			cout << to_binary(c + d) << " vs " << to_binary(posito<nbits,es>(0)) << endl;
...
catch (const sw::universal::posito_arithmetic_exception& err) {
	std::cerr << "Uncaught posito arithmetic exception: " << err.what() << std::endl;
	return EXIT_FAILURE;
}
catch (const sw::universal::posito_internal_exception& err) {
	std::cerr << "Uncaught posito internal exception: " << err.what() << std::endl;
	return EXIT_FAILURE;
}

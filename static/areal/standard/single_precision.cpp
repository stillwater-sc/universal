// single_precision.cpp: comparison tests for areal<32,8> vs IEEE fp32
//
 ...
 * IEEE fp32 (cfloat<32,8>). The key difference:
 ...
 *
 * Single precision provides more headroom than half precision, allowing us
 * to observe how uncertainty propagates through longer iteration sequences.
 */
...
// Taylor series coefficients for ln(1+x) = x - x^2/2 + x^3/3 - x^4/4 + ...
...
int CompareTaylorSeries(const std::string& funcName,
	...
	for (double x : testValues) {
		...
		if (reportTestCases) {
			std::cout << std::setw(8) << funcName << "(" << std::setw(10) << x << "): "
			          << "areal=" << std::setw(14) << arealValue
			          << (isUncertain ? "(u)" : "   ")
			          << " cfloat=" << std::setw(14) << cfloatValue
			          << " ref=" << std::setw(16) << refValue
			          << " aerr=" << std::scientific << std::setw(12) << arealError
			          << " cerr=" << std::setw(12) << cfloatError << std::fixed << '\n';
		}
	}
...
int CompareHarmonicSeries(int terms, bool reportTestCases) {
	...
	std::cout << "  Reference:    " << std::setprecision(12) << refSum << '\n';
...
int CompareNewtonSqrt(double value, int maxIter, bool reportTestCases) {
	...
	if (reportTestCases) {
		std::cout << "Newton sqrt(" << value << ") with " << maxIter << " iterations (areal<"
		          << nbits << "," << es << ">):\n";
		std::cout << "  Reference:    " << std::setprecision(12) << refSqrt << '\n';
		std::cout << "  Areal:        " << arealValue << (isUncertain ? " (uncertain)" : " (exact)") << '\n';
		std::cout << "  Cfloat:       " << cfloatValue << '\n';
		std::cout << "  Areal error:  " << std::scientific << arealError << std::fixed << '\n';
		std::cout << "  Cfloat error: " << std::scientific << cfloatError << std::fixed << '\n';
...
// Kahan summation comparison - demonstrates how areal tracks uncertainty vs cfloat rounding
template<size_t nbits, size_t es, typename bt>
int CompareKahanSummation(int terms, bool reportTestCases) {
	using Areal  = areal<nbits, es, bt>;
	using Cfloat = cfloat<nbits, es, bt, true, false, false>;

	// Compute reference using double with Kahan
	...
	// Standard summation with areal
	Areal arealSum = 0;
	for (int i = 1; i <= terms; ++i) {
		arealSum += Areal(1) / Areal(i);
	}
	bool arealUncertain = (arealSum.block(0) & 1) != 0;

	// Standard summation with cfloat
	Cfloat cfloatSum = 0;
	for (int i = 1; i <= terms; ++i) {
		cfloatSum += Cfloat(1) / Cfloat(i);
	}
	...
	if (reportTestCases) {
		std::cout << "Standard vs Kahan summation H(" << terms << ") with areal<"
		          << nbits << "," << es << ">:\n";
		std::cout << "  Kahan ref:    " << std::setprecision(12) << refSum << '\n';
		std::cout << "  Areal:        " << arealValue << (arealUncertain ? " (uncertain)" : " (exact)") << '\n';
		std::cout << "  Cfloat:       " << cfloatValue << '\n';
		std::cout << "  Areal error:  " << std::scientific << arealError << std::fixed << '\n';
		std::cout << "  Cfloat error: " << std::scientific << cfloatError << std::fixed << '\n';
		std::cout << std::setprecision(6);
...
try {
	...
	std::string test_suite  = "areal<32,8> vs fp32 comparison";
	std::string test_tag    = "single precision comparison";
	bool reportTestCases    = false;
	int nrOfFailedTestCases = 0;

	ReportTestSuiteHeader(test_suite, reportTestCases);

	// Single precision: areal<32,8> matches IEEE fp32 (1 sign + 8 exp + 23 fraction)
	// Note: areal has ubit taking 1 fraction bit, so effective fraction is 22 bits
	...
	// Taylor series for sin with 10 terms (higher order for single precision)
	...
	// Taylor series for cos with 10 terms
	auto cosCoeffs = cos_taylor_coefficients(10);
	...
	std::vector<double> expValues = {0.0, 0.1, 0.5, 1.0, 2.0, 3.0, 5.0};
	...
	// Taylor series for ln(1+x) with 20 terms
	std::vector<double> lnValues = {0.01, 0.1, 0.25, 0.5, 0.75, 0.9};
	auto lnCoeffs = ln1p_taylor_coefficients(20);
	nrOfFailedTestCases += CompareTaylorSeries<nbits, es, bt>("ln1p", lnCoeffs, lnValues,
		[](double x) { return std::log1p(x); }, reportTestCases);
	...
	CompareNewtonSqrt<nbits, es, bt>(1000.0, 15, reportTestCases);

	// Kahan summation comparison
	CompareKahanSummation<nbits, es, bt>(10000, reportTestCases);
	...
	{
		// Taylor series for exp with 10 terms
		std::vector<double> expValues = {0.0, 0.5, 1.0, 2.0, 3.0};
		auto expCoeffs = exp_taylor_coefficients(10);
		nrOfFailedTestCases += CompareTaylorSeries<nbits, es, bt>("exp", expCoeffs, expValues,
			[](double x) { return std::exp(x); }, reportTestCases);
	}
	{
	...
	}
	{
		...
		std::vector<double> lnValues = {0.01, 0.1, 0.25, 0.5, 0.75};
		auto lnCoeffs = ln1p_taylor_coefficients(15);
		nrOfFailedTestCases += CompareTaylorSeries<nbits, es, bt>("ln1p", lnCoeffs, lnValues,
			[](double x) { return std::log1p(x); }, reportTestCases);
	}
	{
	...
	{
		CompareNewtonSqrt<nbits, es, bt>(2.0, 8, true);
		CompareNewtonSqrt<nbits, es, bt>(1000.0, 10, true);
	}
#endif
	...
	{
		CompareHarmonicSeries<nbits, es, bt>(10000, reportTestCases);
		CompareKahanSummation<nbits, es, bt>(10000, true);
	}
#endif
...
#if REGRESSION_LEVEL_4
	{
		...
		for (double x = -3.14159; x <= 3.14159; x += 0.05) {
			manyValues.push_back(x);
		}
		auto sinCoeffs = sin_taylor_coefficients(12);

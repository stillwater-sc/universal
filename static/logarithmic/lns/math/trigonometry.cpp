...
void my_sincospif(float a, float *sp, float *cp)
{
	...
	/* IEEE-754: sinPi(+n) is +0 and sinPi(-n) is -0 for native floats n */
	if (a == floorf(a)) s = az;
	*sp = s;
	*cp = c;
}
...
try {
	...
	std::string test_suite  = "lns<> mathlib trigonometry validation";
...
//	GenerateTestCase<256, 5, float>(m_pi);
	...
	nrOfFailedTestCases += ReportTestResult(VerifySine   < lns<8, 2> >(reportTestCases), "lns<8,2>", "sin");
	nrOfFailedTestCases += ReportTestResult(VerifyCosine < lns<8, 2> >(reportTestCases), "lns<8,2>", "cos");
	nrOfFailedTestCases += ReportTestResult(VerifyTangent< lns<8, 2> >(reportTestCases), "lns<8,2>", "tan");
	...
	nrOfFailedTestCases += ReportTestResult(VerifySine   < lns<16, 5> >(reportTestCases), "lns<16,5>", "sin");
	nrOfFailedTestCases += ReportTestResult(VerifyCosine < lns<16, 5> >(reportTestCases), "lns<16,5>", "cos");
	nrOfFailedTestCases += ReportTestResult(VerifyTangent< lns<16, 5> >(reportTestCases), "lns<16,5>", "tan");

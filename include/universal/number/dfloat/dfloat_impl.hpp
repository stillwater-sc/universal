#pragma once
// dfloat_impl.hpp: implementation of an arbitrary configuration fixed-size decimal floating-point representation
//
...
#ifndef DFLOAT_THROW_ARITHMETIC_EXCEPTION
...
/*
 * decimal floating-point dfloat offers denorms, gradual overflow, and 
...
/// <summary>
/// An arbitrary, fixed-size decimal floating-point number with configurable gradual under/overflow and saturation/non-saturation arithmetic.
...
public:
	...
	static_assert(special, "when es == 1, dfloat must have both subnormals and supernormals");
	...
	constexpr dfloat(const std::string& stringRep) { /* assign(stringRep); */ }

#ifdef LATER
	/// <summary>
	/// construct an dfloat from a native type, specialized for size
	/// </summary>
	/// <param name="iv">initial value to construct</param>
	constexpr dfloat(signed char iv)                    noexcept : _block{} { *this = iv; }
	constexpr dfloat(short iv)                          noexcept : _block{} { *this = iv; }
	constexpr dfloat(int iv)                            noexcept : _block{} { *this = iv; }
	constexpr dfloat(long iv)                           noexcept : _block{} { *this = iv; }
	constexpr dfloat(long long iv)                      noexcept : _block{} { *this = iv; }
	constexpr dfloat(char iv)                           noexcept : _block{} { *this = iv; }
	constexpr dfloat(unsigned short iv)                 noexcept : _block{} { *this = iv; }
	constexpr dfloat(unsigned int iv)                   noexcept : _block{} { *this = iv; }
	constexpr dfloat(unsigned long iv)                  noexcept : _block{} { *this = iv; }
	constexpr dfloat(unsigned long long iv)             noexcept : _block{} { *this = iv; }
	CONSTEXPRESSION dfloat(float iv)                    noexcept : _block{} { *this = iv; }
	CONSTEXPRESSION dfloat(double iv)                   noexcept : _block{} { *this = iv; }

	// assignment operators
	constexpr dfloat& operator=(signed char rhs)        noexcept { return convert_signed_integer(rhs); }
	constexpr dfloat& operator=(short rhs)              noexcept { return convert_signed_integer(rhs); }
	constexpr dfloat& operator=(int rhs)                noexcept { return convert_signed_integer(rhs); }
	constexpr dfloat& operator=(long rhs)               noexcept { return convert_signed_integer(rhs); }
	constexpr dfloat& operator=(long long rhs)          noexcept { return convert_signed_integer(rhs); }

	constexpr dfloat& operator=(char rhs)               noexcept { return convert_unsigned_integer(rhs); }
	constexpr dfloat& operator=(unsigned short rhs)     noexcept { return convert_unsigned_integer(rhs); }
	constexpr dfloat& operator=(unsigned int rhs)       noexcept { return convert_unsigned_integer(rhs); }
	constexpr dfloat& operator=(unsigned long rhs)      noexcept { return convert_unsigned_integer(rhs); }
	constexpr dfloat& operator=(unsigned long long rhs) noexcept { return convert_unsigned_integer(rhs); }

	CONSTEXPRESSION dfloat& operator=(float rhs)        noexcept { return convert_ieee754(rhs); }
	CONSTEXPRESSION dfloat& operator=(double rhs)       noexcept { return convert_ieee754(rhs); }

	// guard long double support to enable ARM and RISC-V embedded environments
#if LONG_DOUBLE_SUPPORT
	CONSTEXPRESSION dfloat(long double iv)  noexcept : _block{} { *this = iv; }
	CONSTEXPRESSION dfloat& operator=(long double rhs)  noexcept { return convert_ieee754(rhs); }
	explicit operator long double()               const noexcept { return to_native<long double>(); }
#endif

	// arithmetic operators
	// prefix operator
	inline dfloat operator-() const {
		dfloat tmp(*this);
	...
	dfloat& operator+=(double rhs) {
		return *this += dfloat(rhs);
	}
	dfloat& operator-=(const dfloat& rhs) {
		if constexpr (dfloat_trace_sub) std::cout << "---------------------- SUB -------------------" << std::endl;
		if (rhs.isnan()) 
			return *this += rhs;
		else 
			return *this += -rhs;
	...
	dfloat& operator*=(const dfloat& rhs) {
		if constexpr (dfloat_trace_mul) std::cout << "---------------------- MUL -------------------\n";
		// special case handling of the inputs
#if CFLOAT_THROW_ARITHMETIC_EXCEPTION
		if (isnan(NAN_TYPE_SIGNALLING) || rhs.isnan(NAN_TYPE_SIGNALLING)) {
			throw dfloat_operand_is_nan{};
		...
		if constexpr (dfloat_trace_mul) std::cout << to_binary(a) << " : " << a << " *\n" << to_binary(b) << " : " << b << " =\n" << to_binary(product) << " : " << product << '\n';
	...
	dfloat& operator/=(const dfloat& rhs) {
		if constexpr (dfloat_trace_div) std::cout << "---------------------- DIV -------------------" << std::endl;
		...
		// snan / snan = snan
#if CFLOAT_THROW_ARITHMETIC_EXCEPTION
		if (rhs.iszero()) throw dfloat_divide_by_zero();
		if (rhs.isnan()) throw dfloat_divide_by_nan();
		if (isnan()) throw dfloat_operand_is_nan();
#else
		if (isnan(NAN_TYPE_SIGNALLING) || rhs.isnan(NAN_TYPE_SIGNALLING)) {
			setnan(NAN_TYPE_SIGNALLING);
			return *this;
		...
		if constexpr (dfloat_trace_div) std::cout << to_binary(a) << " : " << a << " /\n" << to_binary(b) << " : " << b << " =\n" << to_binary(quotient) << " : " << quotient << '\n';
	...
	constexpr dfloat& setbits(uint64_t raw_bits) noexcept {
		if constexpr (0 == nrBlocks) {
			return *this;
		}
		else if constexpr (1 == nrBlocks) {
	...
	/// </summary>
	/// <returns>reference to this dfloat object</returns>
	constexpr dfloat& flip() noexcept { // in-place one's complement
		for (unsigned i = 0; i < nrBlocks; ++i) {
	...
	CONSTEXPRESSION dfloat& assign(const std::string& str) noexcept {
		...
		if (nrDots != 2) {
			std::cerr << "number of segment delimiters in string is " << nrDots << " and needs to be 2 for a dfloat<>\n";
			return *this;
		}
...
//		return !e.iszero() && !isinf() && !isnan();  // old definition that included the supernormals but excluded the extreme encodings
	...
	constexpr bool inrange(NativeReal v) {
		// the valid range for this dfloat includes the interval between 
		// maxpos and the value that would round down to maxpos
		bool bIsInRange = true;		
		if (v > 0) {
			dfloat c(SpecificValue::maxpos);
			dfloat<nbits + 1, es, BlockType, hasSubnormals, hasSupernormals, isSaturating> d;
			d = NativeReal(c);
			++d;
			if (v >= NativeReal(d)) bIsInRange = false;
		}
		else {
			dfloat c(SpecificValue::maxneg);
			dfloat<nbits + 1, es, BlockType, hasSubnormals, hasSupernormals, isSaturating> d;
			d = NativeReal(c);
			--d;
			if (v <= NativeReal(d)) bIsInRange = false;
...
//				std::cout << "msb : " << msb << " : fhbits : " << fhbits << " : " << to_binary(significant, true) << std::endl;
	...
	// transform an dfloat to a native C++ floating-point. We are using the native
	...
	// convert a dfloat to a blocktriple with the fraction format 1.ffff
	...
	// most dfloat<->blocktriple cases being efficient as the block types are aligned.
	// The relationship between the source dfloat and target blocktriple is not
	// arbitrary, enforce it by setting the blocktriple fbits to the dfloat's (nbits - es - 1)
	constexpr void normalize(blocktriple<fbits, BlockTripleOperator::REP, bt>& tgt) const {
		...
		if (isnan()) {
			...
			else { // it is a subnormal encoding in this target dfloat
	...
	// normalize a dfloat to a blocktriple used in add/sub, which has the form 00h.fffff
	...
	constexpr void normalizeAddition(blocktriple<fbits, BlockTripleOperator::ADD, bt>& tgt) const {
		...
		if (isnan()) {
			...
			if (isnormal()) {
				...
				if (isdenormal()) { // it is a subnormal encoding in this target dfloat
					if constexpr (hasSubnormals) {
					...
					else {  // this dfloat has no subnormals
						tgt.setzero(tgt.sign()); // preserve the sign
					}
				}
				else {
					// by design, a dfloat is either normal, subnormal, or supernormal, so this else clause is by deduction covering a supernormal
					if constexpr (hasSupernormals) {
						if constexpr (fbits < 64 && BlockTripleConfiguration::rbits < (64 - fbits)) {
							uint64_t raw = fraction_ull();
							raw |= (1ull << fbits); // add the hidden bit
					...
					else {  // this dfloat has no supernormals and thus this represents a nan, signalling or quiet determined by the sign
						tgt.setnan(tgt.sign());
					}			
				}
			}
	...
	// Normalize a dfloat to a blocktriple used in mul, which has the form 0'00001.fffff
	...
	constexpr void normalizeMultiplication(blocktriple<fbits, BlockTripleOperator::MUL, bt>& tgt) const {
		...
		if (isnan()) {
			...
			if (isnormal() || issupernormal()) {
				...
				// it is a subnormal encoding in this target dfloat
				if constexpr (hasSubnormals) {
					if constexpr (fbits < 64) {
						uint64_t raw = fraction_ull();
						int shift = MIN_EXP_NORMAL - scale;
	...
	// normalize a dfloat to a blocktriple used in div, which has the form 0'00000'00001.fffff
...
protected:
	...
	// convert an unsigned integer into a dfloat
	// TODO: this method does not protect against being called with a signed integer
	template<typename Ty>
	constexpr dfloat& convert_unsigned_integer(const Ty& rhs) noexcept {
		clear();
		...
		int exponent = msb;
		// remove the MSB as it represents the hidden bit in the dfloat representation
	...
	// convert a signed integer into a dfloat
	// TODO: this method does not protect against being called with a signed integer
	template<typename Ty>
	constexpr dfloat& convert_signed_integer(const Ty& rhs) noexcept {
		clear();
		if (0 == rhs) return *this;
		bool s = (rhs < 0);
...
public:
	...
	CONSTEXPRESSION dfloat& convert_ieee754(Real rhs) noexcept {
		if constexpr (nbits == 32 && es == 8 && sizeof(Real) == 4) {
			// we CANNOT use the native conversion to float as dfloats have supernormals
			...
			// when our dfloat is a perfect match to single precision IEEE-754
			bool s{ false };
			...
			if (rawExponent == ieee754_parameter<Real>::eallset) { // nan and inf need to be remapped
				if (rawFraction == (ieee754_parameter<Real>::fmask & ieee754_parameter<Real>::snanmask) ||
					...
					setnan(NAN_TYPE_SIGNALLING);
					//setsign(s);  a dfloat encodes a signalling nan with sign = 1, and a quiet nan with sign = 0
					return *this;
				}
			...
			// when our dfloat is a perfect match to double precision IEEE-754
			bool s{ false };
			uint64_t rawExponent{ 0 };
			uint64_t rawFraction{ 0 };
			...
			// with. 
			// For each specific dfloat configuration, you can work out these rounding cusps
			...
			// comparing their scale to the max scale this dfloat encoding can represent.
			...
			// input is normal, dfloat is normal           <-- rounding can happen with native ieee-754 bits
			// input is normal, dfloat is subnormal
			// input is subnormal, dfloat is normal
			// input is subnormal, dfloat is subnormal
			...
			// in the target dfloat: these are constexpressions and guard the shifts
			// input fbits >= dfloat fbits                 <-- need to round
			// input fbits < dfloat fbits                  <-- no need to round
			...
			if constexpr (fbits < ieee754_parameter<Real>::fbits) {
				// this is the common case for dfloats that are smaller in size compared to single and double precision IEEE-754
...
//						exponent = (exponent < MIN_EXP_SUBNORMAL ? MIN_EXP_SUBNORMAL : exponent); // clip to the smallest subnormal exponent, otherwise the adjustment is off
						...
						// target structure is for example dfloat<8,2>: seef'ffff
					...
					std::cout << "biased exponent   : " << biasedExponent << " : 0x" << std::hex << biasedExponent << std::dec << '\n';
					std::cout << "fraction bits     : " << to_binary(rawFraction, 32, true) << '\n';
					std::cout << "dfloat bits       : " << to_binary(bits, nbits, true) << '\n';
#endif
					setbits(bits);
				}
				else {
				...
				// dfloat<64, 11> = float
				// dfloat<64, 10> = double 
				// can we go from an input subnormal to a dfloat normal? 
				// yes, for example a dfloat<64,11> assigned to a subnormal float

				// map exponent into target dfloat encoding
	...
	//					std::cerr << "rhs is a subnormal : " << to_binary(rhs) << " : " << rhs << '\n';
					...
					// use cases are dfloats like dfloat<80, 11, bt>
...
private:
	...
	friend std::ostream& operator<< (std::ostream& ostr, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& r);
	template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
	friend std::istream& operator>> (std::istream& istr, dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& r);

	template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
	friend bool operator==(const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& lhs, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& rhs);
	template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
	friend bool operator!=(const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& lhs, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& rhs);
	template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
	friend bool operator< (const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& lhs, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& rhs);
...
// convert dfloat to decimal fixpnt string, i.e. "-1234.5678"
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
std::string to_decimal_fixpnt_string(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& value, long long precision) {
	constexpr unsigned fbits = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>::fbits;
	constexpr unsigned bias = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>::EXP_BIAS;
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
std::string to_string(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& value, long long precision) {
	constexpr unsigned fbits = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>::fbits;
	std::stringstream str;
	if (value.iszero()) {
		str << '0';
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline std::ostream& operator<<(std::ostream& ostr, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& v) {
...
// istream input: TBD
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline std::istream& operator>>(std::istream& istr, const dfloat<nbits,es,bt,hasSubnormals,hasSupernormals,isSaturating>& v) {
	istr >> v._fraction;
	return istr;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> ulp(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& a) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> b(a);
	return ++b - a;
}

// transform dfloat to a binary representation
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline std::string to_binary(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& number, bool nibbleMarker = false) {
	std::stringstream s;
	s << "0b";
...
// transform a dfloat into a triple representation
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline std::string to_triple(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& number, bool nibbleMarker = true) {
	std::stringstream s;
//	blocktriple<dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>::fbits, BlockTripleOperator::REP, bt> triple;
...
// Magnitude of a dfloat (equivalent to turning the sign bit off).
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
abs(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& v) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> a(v);
	a.setsign(false);
	return a;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>
fabs(dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> v) {
	return abs(v);
}
...
template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
inline bool operator==(const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& lhs, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& rhs) {
...
template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
inline bool operator!=(const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& lhs, const dfloat<nnbits,nes,nbt,nsub,nsup,nsat>& rhs) { return !operator==(lhs, rhs); }
template<unsigned nnbits, unsigned nes, typename nbt, bool nsub, bool nsup, bool nsat>
inline bool operator< (const dfloat<nnbits, nes, nbt, nsub, nsup, nsat>& lhs, const dfloat<nnbits, nes, nbt, nsub, nsup, nsat>& rhs) {
#ifdef LATER
	if (lhs.isnan() || rhs.isnan()) return false;
	...
	if constexpr (nsub) {
		dfloat<nnbits, nes, nbt, nsub, nsup, nsat> diff = (lhs - rhs);
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator-(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> diff(lhs);
	diff -= rhs;
	return diff;
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator*(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> mul(lhs);
	mul *= rhs;
	return mul;
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator/(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> ratio(lhs);
	ratio /= rhs;
	return ratio;
}

/// binary dfloat - literal arithmetic operators

template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(float lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(double lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(int lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(unsigned int lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(long long lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(unsigned long long lhs, const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& rhs) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> sum(lhs);
	sum += rhs;
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, float rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
}
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, double rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, unsigned int rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, long long rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> operator+(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, unsigned long long rhs) {
	using Cfloat = dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>;
	Cfloat sum(lhs);
	sum += Cfloat(rhs);
	return sum;
...
// dfloat - literal float logic operators
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator==(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, float rhs) {
...
// dfloat - literal double logic operators
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator==(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, double rhs) {
...
#if LONG_DOUBLE_SUPPORT
// dfloat - literal long double logic operators
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator==(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, long double rhs) {
...
// dfloat - literal int logic operators
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator==(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	return operator==(lhs, dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs));
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator!=(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	return operator!=(lhs, dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs));
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator> (const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	return operator<(dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs), lhs);
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator<=(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	return operator<(lhs, dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs)) || operator==(lhs, dfloat<nbits, es, bt>(rhs));
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator>=(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, int rhs) {
	return !operator<(lhs, dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs));
...
// dfloat - long long logic operators
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator==(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, long long rhs) {
	return operator==(lhs, dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>(rhs));
}
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline bool operator!=(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& lhs, long long rhs) {
...
template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> frexp(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& x, int* exp) {
	*exp = x.scale();
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> fraction(x);
	fraction.setexponent(0);
	return fraction;
}

template<unsigned nbits, unsigned es, typename bt, bool hasSubnormals, bool hasSupernormals, bool isSaturating>
inline dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> ldexp(const dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating>& x, int exp) {
	dfloat<nbits, es, bt, hasSubnormals, hasSupernormals, isSaturating> result(x);
	int xexp = x.scale();

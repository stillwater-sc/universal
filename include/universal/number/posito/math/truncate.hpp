#pragma once
// truncate.hpp: truncation functions (trunc, round, floor, and ceil) for positos
//
...
posito<nbits,es> trunc(posito<nbits,es> x) {
	return posito<nbits,es>(std::trunc(double(x)));
}

// Round to nearest: returns the integral value that is nearest to x, with halfway cases rounded away from zero
template<unsigned nbits, unsigned es>
posito<nbits,es> round(posito<nbits,es> x) {
	return posito<nbits,es>(std::round(double(x)));
}

// Round x downward, returning the largest integral value that is not greater than x
template<unsigned nbits, unsigned es>
posito<nbits,es> floor(posito<nbits,es> x) {
	return posito<nbits,es>(std::floor(double(x)));
}

// Round x upward, returning the smallest integral value that is greater than x
template<unsigned nbits, unsigned es>
posito<nbits,es> ceil(posito<nbits,es> x) {
	return posito<nbits,es>(std::ceil(double(x)));
}

...
namespace sw { namespace universal {
	...
	template<typename TestType, typename GoldenType>
	void executeBinary(int opcode, GoldenType da, GoldenType db, GoldenType& dc, 
						const TestType& testa, const TestType& testb, TestType& testc) {
		dc = 0.0;
		switch (opcode) {
		case OPCODE_ADD:
			testc = testa + testb;
	...
	int VerifyBinaryOperatorThroughRandoms(bool reportTestCases, int opcode, uint32_t nrOfRandoms) {
		...
		for (unsigned i = 1; i < nrOfRandoms; i++) {
			posit<nbits, es> testa, testb, testc;
			testa.setbits(distr(eng));
			testb.setbits(distr(eng));
			posito<nbits, es> da, db, dc;
			da.setBitblock(testa.get());
			db.setBitblock(testb.get());
			...
			try {
				executeBinary(opcode, da, db, dc, testa, testb, testc);
			}
			...
			executeBinary(opcode, da, db, dc, testa, testb, testc);
#endif
			if (testc.bits() != dc.bits()) {
				nrOfFailedTests++;
				if (reportTestCases) ReportSpecializedBinaryArithmeticError("FAIL", operation_string, testa, testb, testc, dc);
			}
			else {
	...
	// We will then execute the binary operator nrOfRandom combinations.
	template<size_t nbits, size_t es>
	int VerifyUnaryOperatorThroughRandoms(bool reportTestCases, int opcode, uint32_t nrOfRandoms) {
		std::string operation_string;
		...
		int nrOfFailedTests = 0;
		for (unsigned i = 1; i < nrOfRandoms; i++) {
			posit<nbits, es> testa, testc;
			testa.setbits(distr(eng));
			if (sqrtOperator && testa < 0) testa = -testa;
			posito<nbits, es> da, dc;
			da.setBitblock(testa.get());
			// in case you have numeric_limits<long double>::digits trouble... this will show that
			//std::cout << "sizeof da: " << sizeof(da) << " bits in significant " << (std::numeric_limits<long double>::digits - 1) << " value da " << da << " at index " << ia << " testa " << testa << std::endl;
#if POSIT_THROW_ARITHMETIC_EXCEPTION
			try {
				executeUnary(opcode, da, dc, testa, testc);
			}
			catch (const posit_arithmetic_exception& err) {
				if (testa.isnar()) {
			...
			if (testc.bits() != dc.bits()) {
				nrOfFailedTests++;
				if (reportTestCases) ReportUnaryArithmeticError("FAIL", operation_string, testa, testc, dc);
			}
			else {

...
template <typename Scalar> class ConstRowProxy {
public:
  ConstRowProxy(typename std::vector<Scalar>::const_iterator iter)
    : _iter(iter)
  {
  }
...
template <typename Scalar> class RowProxy {
public:
  RowProxy(typename std::vector<Scalar>::iterator iter)
    : _iter(iter)
  {
  }
...
public:
  ...
  RowProxy<Scalar> operator[](size_t i)
  {
    typename std::vector<Scalar>::iterator it =
      data.begin() + int64_t(i) * int64_t(_n);
  ...
  ConstRowProxy<Scalar> operator[](size_t i) const
  {
    typename std::vector<Scalar>::const_iterator it =
      data.begin() + static_cast<int64_t>(i * _n);
  ...
  matrix& operator+=(const matrix& rhs)
  {
    ...
    for (size_type e = 0; e < _m * _n; ++e) { data[e] += rhs.data[e]; }
    return *this;
  }
  ...
  matrix& operator-=(const matrix& rhs)
  {
    ...
    if (_m != rhs._m || _n != rhs._n) {
      std::cerr
        << "Element-wise matrix difference received incompatible matrices ("
        << _m << ", " << _n << ") -= (" << rhs._m << ", " << rhs._n << ")\n";
      return *this;   // return without changing
    }
    for (size_type e = 0; e < _m * _n; ++e) { data[e] -= rhs.data[e]; }
  ...
  matrix& operator*=(const Scalar& a)
  {
    ...
    for (size_type e = 0; e < _m * _n; ++e) { data[e] *= a; }
    return *this;
  }
  ...
  inline void setzero()
  {
    for (auto& elem: data) elem = Scalar(0);
  }
...
template <typename Scalar> inline size_t num_rows(const matrix<Scalar>& A)
{
  return A.rows();
}
template <typename Scalar> inline size_t num_cols(const matrix<Scalar>& A)
{
  return A.cols();
}
template <typename Scalar>
inline std::pair<size_t, size_t> size(const matrix<Scalar>& A)
{
  return A.size();
}

// ostream operator: no need to declare as friend as it only uses public
// interfaces
template <typename Scalar>
...
inline std::string hex_format(const matrix<sw::universal::posit<nbits, es>>& A)
{
  ...
  size_t n = A.cols();
  for (size_t i = 0; i < m; ++i) {
    for (size_t j = 0; j < n; ++j) { ostr << hex_format(A(i, j)) << " "; }
    ostr << '\n';
  }
  return ostr.str();
}
...
vector<Scalar> operator*(const matrix<Scalar>& A, const vector<Scalar>& x)
{
  ...
  for (size_t i = 0; i < A.rows(); ++i) {
    b[i] = Scalar(0);
    for (size_t j = 0; j < A.cols(); ++j) { b[i] += A(i, j) * x[j]; }
  }
  return b;
}
...
template <size_t nbits, size_t es>
vector<posit<nbits, es>> operator*(const matrix<posit<nbits, es>>& A,
                                   const vector<posit<nbits, es>>& x)
{
  constexpr size_t capacity = 20;   // FDP for vectors < 1,048,576 elements
  vector<posit<nbits, es>> b(A.rows());
  for (size_t i = 0; i < A.rows(); ++i) {
    quire<nbits, es, capacity> q;
    for (size_t j = 0; j < A.cols(); ++j) { q += quire_mul(A(i, j), x[j];
    }
    convert(q.to_value(),
            b[i]);   // one and only rounding step of the fused-dot product
  }
  return b;
}
*/
...
{
  if (A.cols() != B.rows())
    throw matmul_incompatible_matrices(
      incompatible_matrices(A.rows(), A.cols(), B.rows(), B.cols(), "*")
        .what());
  size_t rows = A.rows();
  size_t cols = B.cols();
  size_t dots = A.cols();
  ...
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      Scalar e = Scalar(0);
      for (size_t k = 0; k < dots; ++k) { e += A(i, k) * B(k, j); }
      C(i, j) = e;
    }
  }
  return C;
...
template <size_t nbits, size_t es>
matrix<posit<nbits, es>> operator*(const matrix<posit<nbits, es>>& A,
                                   const matrix<posit<nbits, es>>& B)
{
  constexpr size_t capacity = 20;   // FDP for vectors < 1,048,576 elements
  if (A.cols() != B.rows())
    throw matmul_incompatible_matrices(
  ...
  for (size_t i = 0; i < rows; ++i) {
    for (size_t j = 0; j < cols; ++j) {
      quire<nbits, es, capacity> q;
      for (size_t k = 0; k < dots; ++k) { q += quire_mul(A(i, k), B(k, j)); }
      convert(q.to_value(),
              C(i, j));   // one and only rounding step of the fused-dot product
    }
  }
  return C;
}

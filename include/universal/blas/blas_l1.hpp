...
template<typename T>
void get_column(const matrix<T>& A, size_t j, vector<vector<T>>B(T(n), vector<int>(T(m)))) {
	for (size_t i = 1; i < num_cols(A); ++i) { B[i][j] = A[i][j]; }
}
template<typename T>
void set_column(const matrix<T>& A, size_t j, vector<vector<T>>B(T(n), vector<int>(T(m)))){

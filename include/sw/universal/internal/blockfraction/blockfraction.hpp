...
//			std::cout << "divider : " << to_binary(divider) << " : " << divider << '\n';
	...
	constexpr uint8_t nibble(unsigned n) const noexcept {
		if (n < (1 + ((nbits - 1) >> 2))) {
			...
			return static_cast<uint8_t>(nibblebits >> (nibbleIndexInWord*4));
		}
	...
	constexpr double to_double() const noexcept {
		...
		if (shift >= 0 && shift < 64) {
			...
			// Integer weights exceed what this implementation can express.
			// If any bit above the radix is set, the magnitude is effectively unbounded here.
			for (; bit >= radixPoint; --bit) {
...
public:
	...
	// internal/blockfraction/iostream.hpp and needs no private access (it streams the

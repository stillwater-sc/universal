...
public:
	...
	static constexpr int min_exponent10 = sw::universal::decimal_min_exponent10(LNS::min_exponent);
	static constexpr int max_exponent = sw::universal::saturate_exponent_to_int(LNS::max_exponent);
	// from the WIDE exponent, not from the saturated int above: the binary bound can
	// exceed int while the decimal one still fits (#1609 review).
	static constexpr int max_exponent10 = sw::universal::decimal_max_exponent10(LNS::max_exponent);

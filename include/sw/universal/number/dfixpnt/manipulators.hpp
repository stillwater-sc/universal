...
// native semantic representation: radix-10, shows decimal digits with point
template<unsigned ndigits, unsigned radix, DecimalEncoding encoding, bool arithmetic, typename bt>
std::string to_native(const dfixpnt<ndigits, radix, encoding, arithmetic, bt>& v, bool = false) {
	std::stringstream s;
	s << (v.sign() ? '-' : '+');
	// integer digits (MSD to LSD)
	for (int i = static_cast<int>(ndigits) - 1; i >= static_cast<int>(radix); --i) {

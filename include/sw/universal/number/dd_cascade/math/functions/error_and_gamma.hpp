...
namespace sw { namespace universal {
	...
	inline dd_cascade erf(const dd_cascade& x) {
		return dd_cascade(std::erf(double(x.high())));
	}

	// Compute the complementary error function: 1 - erf(x)
    inline dd_cascade erfc(const dd_cascade& x) {
	    return dd_cascade(std::erfc(double(x.high())));
	}

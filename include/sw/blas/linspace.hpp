...
namespace sw { namespace blas {
	...
	vector<Scalar> arange(int64_t start, int64_t stop, int64_t step = 1) {
		if (start > stop) return vector<Scalar>(0);
		vector<Scalar> v;
	...
	*/
	template<typename Scalar>
	vector<Scalar> linspace(const Scalar& start, const Scalar& stop, size_t steps, bool endpoint = true) {
		if (steps == 0) return vector<Scalar>(0);
		if (steps == 1) return vector<Scalar>(1) = start;
		vector<Scalar> v(steps);
	...
	*/
	template<typename Scalar>
	vector<Scalar> logspace(const Scalar& start, const Scalar& stop, size_t steps, bool endpoint = true, const Scalar& base = Scalar(10.0)) {
		using std::pow;
		if (steps == 0) return vector<Scalar>(0);
		if (steps == 1) return vector<Scalar>(1) = pow(base, start);
	...
	*/
	template<typename Scalar>
	vector<Scalar> geomspace(const Scalar& start, const Scalar& stop, size_t steps, bool endpoint = true, const Scalar& base = Scalar(10.0)) {
		using std::pow;
		if (steps == 0) return vector<Scalar>(0);
		if (steps == 1) return vector<Scalar>(1) = pow(base, start);

...
### On expansion density (universal#1364, resolved)

An earlier note here suggested treating a narrow host's `blocks * k` payload as
the conservative digit figure, on the theory that `agreed_decimal_digits` might
be crediting how far an expansion *reaches* rather than what it *carries*. That
was wrong, and the reasoning behind it was wrong twice over.

Consecutive block exponents sit more than `k` apart on **every** host, not just
narrow ones -- a depth-32 `e` has 35 of 35 gaps above `k` on double (k=53, gaps
reaching 59) and 29 of 29 on bfloat16. So it was never a narrow-host anomaly; it
is simply the shape of a 0-overlap expansion.

And the payload figure is not a bound on agreement, because an expansion is a
signed **sum**, not a concatenation of bit fields. Six of the thirty components of
a bfloat16 `e` are negative, so a low component borrows from a high one and the
sum's bit pattern is not the union of the components' patterns. The block
exponents carry information as well as the significands.

Checked rather than argued: the depth-32 expansions were rebuilt in exact rational
arithmetic outside this library and compared against a 3000-digit reference
generated by exact integer arithmetic.

| host | digits claimed | independently verified |
|------|----------------|------------------------|
| `bfloat16` | 78 | **78** |
| `half` | 122 | **122** |
| `float` | 263 | **263** |

`agreed_decimal_digits` is reporting correctly. `elastic/elreal/oracle/sweep.cpp`
now guards the property that would actually fail if sparseness were a defect --
that a product of two sparse operands equals the exact dyadic product.

...
try {
	...
	// TODO Phase 2: Add range tests (very large positive, negative values)

	ereal<> x(2.0);

	std::cout << "Testing exponential functions...\n";
	std::cout << "exp(" << x << ") = " << exp(x) << '\n';
	std::cout << "exp2(" << x << ") = " << exp2(x) << '\n';
	std::cout << "exp10(" << x << ") = " << exp10(x) << '\n';
	std::cout << "expm1(0.1) = " << expm1(ereal<>(0.1)) << '\n';
	...
	// TODO Phase 2: Add REGRESSION_LEVEL_1 tests (basic exp at double precision)

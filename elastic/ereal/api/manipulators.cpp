// manipulators.cpp: ereal manipulator surface
//
...
namespace {
	...
	int VerifyManipulatorSurface(bool reportTestCases) {
		...
		const Named rendered[] = {
			...
			{ to_components(v), to_components(w), "to_components" },
	...
	int VerifyRepairedRenderings(bool reportTestCases) {
		...
		fails += expect_true(hex.rfind("ereal[", 0) == 0, "to_hex tags the type", reportTestCases);
		fails += expect_true(hex.find("0x") != std::string::npos,
			"to_hex delegates to the limbs' native hex", reportTestCases);
		fails += expect_true(to_hex(v, false, false).find("0x") == std::string::npos,
			"to_hex forwards hexPrefix=false to the limbs", reportTestCases);
		fails += expect_true(hex.find_first_of("0123456789ABCDEF") != std::string::npos,
			"to_hex carries hex digits", reportTestCases);
		// one rendering per limb
		fails += expect_true(to_hex(Ereal(1.0) / Ereal(3.0)).find(", ") != std::string::npos,
			"a multi-limb value renders every limb", reportTestCases);
		...
		fails += expect_true(!hp.empty() && hp.back() == 'R', "hex_print tags the type", reportTestCases);
		...
		fails += expect_true(cp.rfind("ereal[", 0) == 0, "color_print tags the type", reportTestCases);

		// the special encodings render as themselves rather than as a significand
		fails += expect_true(!is_placeholder(to_hex(Ereal(0.0))), "zero renders as zero", reportTestCases);
		fails += expect_true(!is_placeholder(pretty_print(Ereal(0.0))), "zero is not a placeholder", reportTestCases);

		// a negative value is distinguishable from its magnitude
		fails += expect_true(to_hex(Ereal(-1.5)) != to_hex(Ereal(1.5)),
			"the sign reaches to_hex", reportTestCases);
		fails += expect_true(pretty_print(Ereal(-1.5)) != pretty_print(Ereal(1.5)),
...
try {
	...
	std::string test_suite  = "ereal manipulator surface";
	std::string test_tag    = "ereal manipulators";
...
#if MANUAL_TESTING
	{
		...
		std::cout << "to_components: " << to_components(v) << '\n';

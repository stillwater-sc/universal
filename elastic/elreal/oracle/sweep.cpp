...
namespace {
	...
	// ---- sparse expansions are exact (#1364) --------------------------------
	// A ZBCL's consecutive block exponents are typically more than k apart -- on
	// every host, not just narrow ones: a depth-32 e has 35 of 35 gaps above k on
	// double (k=53, gaps to 59) and 29 of 29 on bfloat16 (k=7, gaps to 13). That
	// looked like it might mean information was being dropped, since the agreeing
	// digits can exceed blocks*k*log10(2).
	//
	// It does not, and the reason is that an expansion is a signed SUM, not a
	// concatenation of bit fields. Six of the thirty components of a bfloat16 e are
	// negative, so a low component borrows from a high one and the sum's bit pattern
	// is not the union of the components' patterns. The block exponents are carrying
	// information too, which is why blocks*k is not a bound on agreement.
	//
	// Verified outside this library: the depth-32 e expansions were rebuilt in exact
	// rational arithmetic and compared against a 3000-digit reference generated by
	// exact integer arithmetic. bfloat16 claimed 78 digits and delivered 78, half
	// 122 and 122, float 263 and 263.
	//
	// What is worth guarding in-tree is the property that would actually fail if
	...
	int VerifySparseExpansionExact(const char* host, bool reportTestCases, int nrTests) {
		...
		for (int t = 0; t < nrTests; ++t) {
			...
			if (digits < 60) {
				if (reportTestCases) std::cout << "    FAIL sparse-product " << host
				                               << " digits=" << digits << '\n';
				++fails;
			}
		}
...
try {
	...
	nrOfFailedTestCases += ReportTestResult(VerifyCancellationExact<double>("double", reportTestCases), "elreal<double> cancellation-stressed accumulation is exact", "cancellation");
	{
		...
		nrOfFailedTestCases += ReportTestResult(VerifySparseExpansionExact<double>("double", reportTestCases, base / 4), "elreal<double> sparse expansions are exact", "sparse");
		nrOfFailedTestCases += ReportTestResult(VerifySparseExpansionExact<float>("float", reportTestCases, base / 4), "elreal<float> sparse expansions are exact", "sparse");
	}

...
### Flagging a behaviour change

Release notes are generated by git-cliff from commit subjects, which cannot say
"this changes what your code returns". When a change alters results that existing
callers would observe, add a trailer to the commit body:
...
git-cliff collects those into a **Behaviour changes** section at the top of the
release notes, above the type groups, with the scope and PR link attached. The
commit still appears in its normal group as well. `BEHAVIOR-CHANGE` is accepted
too.

Use it for anything a caller could notice: changed numeric results, a renamed or
re-aimed alias, a default that moved. Do not use it for internal refactors, new
functionality that breaks nothing, or performance work that leaves results
identical.

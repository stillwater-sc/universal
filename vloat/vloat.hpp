#pragma once
// vloat.hpp: definition of a variable float representation that mimics the posit configuration
//
...
namespace sw {
	namespace unum {
		...
		// Forward definitions
		template<size_t nbits, size_t es> class vloat;
		template<size_t nbits, size_t es> vloat<nbits,es> abs(const vloat<nbits,es>& v);
		...
		public:
			...
			vloat() : _sign(false), _scale(0), _nrOfBits(fbits), _zero(true), _inf(false), _nan(false) {}
			vloat(bool sign, int scale, const bitblock<fbits>& fraction_without_hidden_bit, bool zero = true, bool inf = false) : _sign(sign), _scale(scale), _nrOfBits(fbits), _fraction(fraction_without_hidden_bit), _inf(inf), _zero(zero), _nan(false) {}
			vloat(signed char initial_value) {
				*this = initial_value;
			}
			vloat(short initial_value) {
				*this = initial_value;
			}
			vloat(int initial_value) {
				*this = initial_value;
			}
			vloat(long long initial_value) {
				*this = initial_value;
			}
			vloat(unsigned long long initial_value) {
				*this = initial_value;
			}
			vloat(float initial_value) {
				*this = initial_value;
			}
			vloat(double initial_value) {
				*this = initial_value;
			}
			vloat(long double initial_value) {
				*this = initial_value;
			}
			vloat(const value& rhs) {
				*this = rhs;
			}
			vloat& operator=(const value& rhs) {
				_sign	  = rhs._sign;
				_scale	  = rhs._scale;
			...
			vloat& operator=(signed char rhs) {
				*this = (long long)(rhs);
				return *this;
			}
			vloat& operator=(short rhs) {
				*this = (long long)(rhs);
				return *this;
			}
			vloat& operator=(int rhs) {
				*this = (long long)(rhs);
				return *this;
			}
			vloat& operator=(long long rhs) {
			...
			vloat& operator=(unsigned long long rhs) {
				if (_trace_conversion) std::cout << "---------------------- CONVERT -------------------" << std::endl;
				if (rhs == 0) {
					setToZero();
				}
			...
			vloat& operator=(long double rhs) {
				reset();
				if (_trace_conversion) std::cout << "---------------------- CONVERT -------------------" << std::endl;
			...
			// operators
			vloat operator-() const {				
				return vloat<nbits,es>(!_sign, _scale, _fraction, _zero, _inf);
			}
		...
		private:
			...
			friend std::ostream& operator<< (std::ostream& ostr, const vloat<nnbits,nes>& r);
			template<size_t nnbits, size_t nes>
			friend std::istream& operator>> (std::istream& istr, vloat<nnbits,nes>& r);

			template<size_t nnbits, size_t nes>
			friend bool operator==(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs);
			template<size_t nnbits, size_t nes>
			friend bool operator!=(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs);
			template<size_t nnbits, size_t nes>
			friend bool operator< (const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs);
		...
		inline std::ostream& operator<<(std::ostream& ostr, const vloat<nnbits,nes>& v) {
			if (v._inf) {
				ostr << FP_INFINITE;
			}
			else {
		...
		template<size_t nfbits>
		inline std::istream& operator>> (std::istream& istr, const vloat<nnbits,nes>& v) {
			istr >> v._fraction;
			return istr;
		}

		template<size_t nnbits, size_t nes>
		inline bool operator==(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return lhs._sign == rhs._sign && lhs._scale == rhs._scale && lhs._fraction == rhs._fraction && lhs._nrOfBits == rhs._nrOfBits && lhs._zero == rhs._zero && lhs._inf == rhs._inf; }
		template<size_t nnbits, size_t nes>
		inline bool operator!=(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return !operator==(lhs, rhs); }
		template<size_t nnbits, size_t nes>
		inline bool operator< (const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return lhs.to_long_double() < rhs.to_long_double(); }
		template<size_t nnbits, size_t nes>
		inline bool operator> (const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return  operator< (rhs, lhs); }
		template<size_t nnbits, size_t nes>
		inline bool operator<=(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return !operator> (lhs, rhs); }
		template<size_t nnbits, size_t nes>
		inline bool operator>=(const vloat<nnbits,nes>& lhs, const vloat<nnbits,nes>& rhs) { return !operator< (lhs, rhs); }
		...
		template<size_t nbits, size_t es>
		inline std::string components(const vloat<nbits,es>& v) {
			std::stringstream s;
			if (v.isZero()) {
		...
		template<size_t nbits, size_t es>
		vloat<nfbits> abs(const vloat<nbits,es>& v) {
			return vloat<nbits,es>(false, v.scale(), v.fraction(), v.isZero());
		}

		// add module
		template<size_t nbits, size_t es, size_t abits>
		void module_add(const vloat<nbits,es>& lhs, const vloat<nbits,es>& rhs, value<abits + 1>& result) {

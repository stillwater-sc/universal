...
These are authored directly in `src/content/docs/` and are **not** generated by the sync script. The sync script skips any destination that already exists, so these are safe from overwriting:
...
1. Reads each source `.md` file listed in `FILE_MAP` and `ROOT_FILE_MAP`
2. Extracts the `# H1` heading as YAML frontmatter `title:`
...
5. **Skips** if a hand-written page already exists at the destination

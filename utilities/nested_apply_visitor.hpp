...
{
    inner_applicator(Visitor vis) : vis(vis) {}
    
    template <std::size_t ES>
    void operator()(const es_tag<ES>&) const
    {
        vis.template operator()<Nbits, ES>();
    }
...
struct outer_applicator
  : public boost::static_visitor<void>
{
    outer_applicator(Visitor vis, const Variant2& v2) : vis(vis), v2(v2) {}
    
    template <std::size_t Nbits>
    void operator()(const nbits_tag<Nbits>&) const
    {
        boost::apply_visitor(inner_applicator<Visitor, Nbits>(vis), v2);
    }

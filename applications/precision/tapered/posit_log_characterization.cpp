...
try {
	...
	std::cout << "   the format's fraction, and starts mis-rounding as that headroom runs\n";
	std::cout << "   out -- not at a clean width, but gradually, as more arguments land near\n";
	std::cout << "   a posit rounding boundary that the double can no longer resolve. This\n";
	std::cout << "   is double rounding, not a shortcoming of the polynomial: the errors are\n";
	std::cout << "   1 ulp and they appear where the format is most precise.\n\n";
	std::cout << "   Past the double's range entirely -- posit<64,2>, and posit<56,0> for the\n";
	std::cout << "   value immediately above 1.0, which a double cannot tell from 1.0 -- the\n";
	std::cout << "   result is not off by an ulp but simply wrong.\n\n";

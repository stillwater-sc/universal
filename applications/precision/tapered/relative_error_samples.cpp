...
try {
...

	{
		...
		std::cout << std::setw(36) << "base"
			...
			<< '\n';
		for (int i = 0; i < 24; ++i) {
			float fa = RelativeError(base, ulp);
			Posit pa = RelativeError(Posit(base), Posit(ulp));
			std::cout << to_binary(Posit(base)) << " Posit(base)\n" << to_binary(Posit(ulp)) << " Posit(ulp)\n";

			Cfloat ca = RelativeError(Cfloat(base), Cfloat(ulp));
			Lns la = RelativeError(Lns(base), Lns(ulp));

...
const char* s_sqrt2 = "1.4142135623730950488016887242096980785696718753769480731766797379907324784621070388503875343276"
...
std::string bigint_to_string(const BI& v) {
	std::ostringstream o;
	o << v;
	return o.str();
}
...
std::string decimal_digits(const ZBCL<FpType>& z, int digits) {
	...
	if (v.scale >= 0)
		...
		den <<= (-v.scale);
	bigint p10;
	p10.assign("1" + std::string(static_cast<std::size_t>(digits), '0'));
	const std::string all = bigint_to_string((num * p10) / den);
	const std::size_t ilen =
	    (all.size() > static_cast<std::size_t>(digits)) ? all.size() - static_cast<std::size_t>(digits) : 1;
	return all.substr(0, ilen) + "." + all.substr(ilen);
}

// How many leading DECIMAL DIGITS of a rendered value match the reference.
// The radix point is skipped rather than counted: counting rendered characters
// would report one more than the value actually agrees to.
int matching_digits(const std::string& got) {
	const std::string ref(s_sqrt2);
	std::size_t       n      = 0;
	int               digits = 0;
	while (n < got.size() && n < ref.size() && got[n] == ref[n]) {
		if (got[n] != '.')
			++digits;
		++n;
	}
...
void native_double_baseline() {
	...
	for (int iters : {4, 8, 16, 64}) {
		...
		std::ostringstream o;
		o << std::setprecision(17) << std::fixed << r;
...
void elreal_fixed_precision(const char* host, std::size_t precision, int iterations) {
	...
	std::cout << "   elreal<" << std::left << std::setw(7) << host << "> precision " << std::setw(4) << precision
	          << " blocks " << std::setw(4) << blocks << "  digits " << std::setw(5)
	          << digits_of<FpType>(r.stream(), 1190) << "  " << std::fixed << std::setprecision(1)
...
int main() try {
	...
	std::cout << "   double " << std::fixed << std::setprecision(1) << std::chrono::duration<double>(t1 - t0).count()
	          << "s, float " << std::chrono::duration<double>(t2 - t1).count()
	...
	// is not run automatically -- but a demo that quietly stopped demonstrating its
	// claim would be worse than no demo. Anyone who runs it, by hand or from a future
	// harness, gets a non-zero exit if the numbers stop backing the prose.
	const int df       = digits_of<float>(rf.stream(), 1190);
	const int dd       = digits_of<double>(rd.stream(), 1190);
	int       failures = 0;
	if (df < 1000) {
		std::cout << "   FAIL elreal<float> reached " << df << " digits, expected >= 1000\n";
		++failures;
	}
	if (dd < 1000) {
		std::cout << "   FAIL elreal<double> reached " << dd << " digits, expected >= 1000\n";
		++failures;
	}
	if (agree < 1000) {
		std::cout << "   FAIL only " << agree << " digits match the reference, expected >= 1000\n";
		++failures;
	}

	std::cout << "\n   From part 3:\n"
	          << "     elreal<float>  reached " << df << " digits\n"
	          << "     elreal<double> reached " << dd << " digits\n"

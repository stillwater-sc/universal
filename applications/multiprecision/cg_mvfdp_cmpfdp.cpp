...
// CG using FDP operations for both matrix-vector and compensation calculations
template<typename Matrix, typename Vector, size_t MAX_ITERATIONS = 10>
size_t cg_fdp_fdp(const Matrix& M, const Matrix& A, Vector& b, typename Matrix::value_type tolerance = typename Matrix::value_type(0.00001)) {
	using Scalar = typename Matrix::value_type;
	Scalar residual = Scalar(std::numeric_limits<Scalar>::max());
	std::vector<Scalar> residuals; // store the residual trajectory
	//size_t m = num_rows(A);
...
size_t Experiment(size_t DoF) {
	...
	M = sw::unum::blas::inv(A);
	return cg_fdp_fdp<Matrix, Vector, MAX_ITERATIONS>(M, A, b);
}
...
//	using Scalar = float;
	...
	*/
	...
	size_t itr = cg_fdp_fdp<Matrix, Vector, MAX_ITERATIONS>(M, A, b);
...
#if STRESS
	Experiment<32,2, 200>(64);
	Experiment<64, 3, 100>(64);
	Experiment<128, 4, 100>(64);
	Experiment<256, 5, 100>(64);
#endif // STRESS
